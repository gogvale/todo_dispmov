...
class GroupTasksController < ApplicationController
  ...
  def index
    ...
    render json: GroupTaskBlueprint.render(@group_tasks), status: :ok
  end

  # GET /group_tasks/1
  def show
    render json: GroupTaskBlueprint.render(@group_task)
  end
  ...
  def create
    ...
    if @group_task.save
      render json: GroupTaskBlueprint.render(@group_task), status: :created
    else
  ...
  def destroy
    @group_task.destroy
    render json: { message: 'Task destroy successfully' }, status: :ok
  end
